...
func (rl *ReportListingChild) GetUserReports() []UserReport {
	...
	for i := range rl.Data.UserReports {
		report := UserReport{}
		report.Reason = rl.Data.UserReports[i].([]interface{})[0].(string)
		report.NumOfReports = rl.Data.UserReports[i].([]interface{})[1].(float64)
		report.SnoozeStatus = rl.Data.UserReports[i].([]interface{})[2].(bool)
		report.CanSnooze = rl.Data.UserReports[i].([]interface{})[3].(bool)
